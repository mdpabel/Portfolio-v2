    ...
    <div className='pt-10 w-full'>
      ...
      <ul className='space-y-4'>
        {stats.map((stat, index) => (
          <li
            key={index}
            className='flex justify-between items-center bg-zinc-50 dark:bg-zinc-800/80 py-1 rounded-lg'>
            <span className='font-medium text-sm text-zinc-700 dark:text-zinc-300'>
              {stat.label}
            </span>
            <span className='font-medium text-base text-teal-700 dark:text-teal-400'>
              {stat.value}
            </span>
          </li>
        ))}
